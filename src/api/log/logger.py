...
def init_logger(filepath: str = "./src/api/log/logger_config.yaml") -> None:
    with open(filepath) as f:
        config = cast(dict, yaml.safe_load(f))
        dictConfig(config)


def init_gunicorn_uvicorn_logger(
    filepath: str = "./src/api/log/logger_config.yaml",
) -> None:
    import logging
    ...
    """
    You only need to switch the logger if you start it with gunicorn
    """
